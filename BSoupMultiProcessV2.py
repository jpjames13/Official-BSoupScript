...
class about():
    def __init__(self,file_path):
        ...
        os.chdir(file_path)
        #writing headers
        self.writeAboutHeader(file_path,about_data_fileName)
        self.writeVersionHeader(file_path,version_data_fileName)
        self.writeAboutErrorLogHeader()  
        bigList = []
        versionList = []
        errorList = []
        ...
        for file_type in glob.glob('*about.htm'):
            ...
            try:
                ...
                aboutDataTable =  (self.ReturnAboutTable(appID,soup_))
                #thread1 = threading.Thread(target=self.ReturnAboutTable, args = (appID,soup_))
                #gets version data in table format
                versionDataTable =  (self.ReturnVersionTable(appID,soup_))
                #thread2 = threading.Thread(target=self.ReturnVersionTable, args=(appID,soup_))
                """UPDATE 8/31 SAVES DATA TO LIST TO WRITE TO CSV LATER"""
                bigList.append(aboutDataTable)
                versionList.append(versionDataTable)
                #file name needs to be in quotations in the argument for write to csv method to be in seperate files
                #self.WriteToCsv(aboutDataTable,about_data_fileName)
                #thread3= threading.Thread(target=self.WriteToCsv,args=(thread1,about_data_fileName))
                #uncomment this later !!!!!!!!!!!!!!!!!!!
                #self.WriteToCsv(versionDataTable,version_data_fileName)
                #thread4= threading.Thread(target=self.WriteToCsv,args=(thread2,version_data_fileName))
                
            except IndexError as ex:
                #print("Internal Error 500")
                self.WriteToAboutErrorLog(appID,url,"Internal Error 500")
#                errorList.append(x)
                pass
            except AttributeError as ex:
                #print("oops!", ex)
                self.WriteToAboutErrorLog(appID,url,ex)
#                errorList.append(y)
                pass
...
#                errorList.append(zz)
                pass  
        """WRITES LIST TO CSV"""
        self.WriteToCsv(bigList,about_data_fileName)
        self.WriteToCsv(versionList,version_data_fileName) 
        #self.WriteToAboutErrorLog(errorList)            
        #print("Finished Processing About Info")
        ############################################################################### 
        '''
        Returns Error Table
        '''
    def ReturnErrorTable(self,appid,URL,exceptions):
        table = []
        row = []
        row.append(appid)
        row.append(URL)
        ...
        '''
    def ReturnAboutTable(self,appid,_soup):
        #list variables
        appNames = ""
        ratingstars = []
        ratingstar = ""
    ...
    #==============================================================================
        ...
        #    print(ratings.find('div'))
            ...
            ratingstars = self.ConvertRatings(ratingstar)
        ...
        #appending list to table to be returned
            ...
            row.append(self.kIsBad(ratingstars))        
            row.append(self.kIsBad(alltimeRating))
            row.append(self.kIsBad(last30daysRate))
            row.append(appDescriptions.replace("\n","").replace(",","").replace("<br>",""))
            row.append(self.kIsBad(star1_30day[0]))
            row.append(self.kIsBad(star2_30day[0]))
            row.append(self.kIsBad(star3_30day[0]))
            row.append(self.kIsBad(star4_30day[0]))
            row.append(self.kIsBad(star5_30day[0]))
            row.append(self.kIsBad(star1_AllTime[0]))
            row.append(self.kIsBad(star2_AllTime[0]))
            row.append(self.kIsBad(star3_AllTime[0]))
            row.append(self.kIsBad(star4_AllTime[0]))
            row.append(self.kIsBad(star5_AllTime[0]))
            table.append(row)
    ...
    '''
    def ReturnVersionTable(self,appId,soup_):
        #table variables
    ...
    def WriteToCsv(self,tableInput,fileName):
    ...
    '''
    #write to csv now takes two arguments
    def WriteToAboutErrorLog(self,appId,url,exception):
    ...
    ###############################################################################
    def writeAboutErrorLogHeader(self):
    ...
    '''
    def ConvertRatings(self,noneType):
    ...
    '''
        Write Header Method
    '''
    def writeAboutHeader(self,filepath, fileName):
    ...
    '''
        Write Header Method
    '''
    def writeVersionHeader(self,filePath,FileName):
...
class sdk():
    def __init__(self,_file_path):
        ...
        os.chdir(_file_path)
        self.writeCSVHeader()
        self.writeErrorLogHeader()
    
        for file_type in glob.glob('*sdk.htm'):
            url = _file_path+file_type
            appID = self.getSDKID(url)
            soup = readHTML(file_type)
            try:
                #print('Currently Processing ... '+file_type)
    
                if self.serverError(soup):
                    self.writeServerErrorToErrorLog(url,appID,soup)
                elif self.paidAppsNoSDKInfo(soup):
                    self.writeDotAsNullSDKInfo(soup,appID)
                else:
                    dataTable = (self.returnTable(soup,appID))
                    self.writeToCsv(dataTable)
    
            except Exception as ex:
            	#print("oops!",ex)
            	self.writeGenericErrorToErrorLog(url,appID,'Exception')
            	pass
            except AttributeError as ex:
            	#print("oops!", ex)
            	self.writeGenericErrorToErrorLog(url,appID,'Attribute')
            	pass
            except TypeError as ex:
                #print("oops!",ex)
                self.writeGenericErrorToErrorLog(url,appID, 'Type')
    
        #print('\nFinished Processing SDK Pages')                
    #retrieve SDK-ID
    def getSDKID(self,url):
        appID = url.replace("_about.sdk","")
        return appID
    
    def getAppName(self,soup):
    ...
    def serverError(self,soup):
        ...
        if serverError:
           ...
           return False
    def writeErrorLogHeader(self):
        header = ['App_ID', 'File_Location', 'Error_Description']
        with open('ErrorLog.csv', 'w+') as f:
            dw = csv.DictWriter(f, fieldnames = header)
            dw.writeheader()
    
    def writeServerErrorToErrorLog(self,url, appID, soup):
        errorMessageArray = []
    ...
    def writeGenericErrorToErrorLog(self,url, appId, errorMessage):
    ...
    def paidAppsNoSDKInfo(self,soup):   
    ...
    def returnTable(self,soup,appID):
        ...
        for i in range(len(sdkNamesArray)):
            appNamesArray.append(self.getAppName(soup))
    ...
    def writeToCsv(self,tableInput):
    ...
    def writeDotAsNullSDKInfo(self,soup, appID):
        arrayToPrintForNoSDKInfoApps = [appID,self.getAppName(soup),u"\u2022",u"\u2022",u"\u2022",u"\u2022",u"\u2022"]
        with open('sdk_Info.csv','a') as f:
            writer = csv.writer(f)
